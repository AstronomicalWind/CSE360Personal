...
public class MyQuestionsPage {
	...
	public void show(Stage primaryStage) {
		    ...
		    if (selected != null) {
		        ...
		        for (Question q : questionsManager.getQuestions()) {
		            if (q.getId() == questionId) {
		                q.setFlagged(true);
		                showAlert("Success", "Question flagged for moderation.", Alert.AlertType.INFORMATION);
		                break;
		            }
		        }
		    } else {
		        showAlert("Error", "No question selected to flag.", Alert.AlertType.ERROR);
		    }
		});
		...
		layout.getChildren().addAll(titleLabel, questionListView,
			    deleteQuestionButton, updateQuestionButton, updateQuestionInfoButton,
			    flagQuestionButton, viewAnswersButton, backButton);
